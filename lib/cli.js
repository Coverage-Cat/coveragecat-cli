...
const MCP_SERVERS = {
  ...
  operator: {
    alias: "operator",
    discoveryPath: "/developers/mcp/operator/server-card.json",
    rpcPath: "/api/agent/mcp"
  },
  docs: {
    alias: "docs",
    discoveryPath: "/developers/mcp/server-card.json",
    rpcPath: "/developers/mcp/server"
  }
};
...
Usage:
  ...
  coveragecat mcp <consumer|operator|docs|product> [manifest|server-card|initialize|tools|resources]
...
Global options:
  ...
  --api-version VERSION  Send Coverage-Cat-API-Version on Coverage Cat API requests. Default: v1
...
function mcpCommand(fetchImpl, options, args) {
  ...
  if (!server || !Object.hasOwn(MCP_SERVERS, server)) {
    throw new Error("mcp requires 'consumer', 'operator', 'docs', or 'product' (alias of consumer) as the server.");
  }
  ...
  switch (action) {
    case "manifest":
    case "server-card":
      return jsonRequest(fetchImpl, options, "GET", config.discoveryPath);

    case "initialize":
      return jsonRequest(fetchImpl, options, "POST", config.rpcPath, {
        body: {
          id: "cli-initialize",
          jsonrpc: "2.0",
          method: "initialize"
    ...
    default:
      throw new Error("mcp actions are manifest, server-card, initialize, resources, or tools.");
  }
}
...
function buildHeaders(options, { accept, body, method, pathname }) {
  ...
  if (options.apiVersion && coverageCatApiPath(pathname)) {
...
function coverageCatApiPath(pathname) {
  return (
